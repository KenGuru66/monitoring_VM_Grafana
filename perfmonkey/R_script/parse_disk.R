...
output_df$MpCnt<-rep(paste0("'",mpcnt,"'"),nrow(output_df))
output_df<-cbind(output_df,merged_df[,2])
output_df$PgCnt<-rep(paste0("'",pgcnt,"'"),nrow(output_df))
output_df$LdCnt<-rep(paste0("'",ldcnt,"'"),nrow(output_df))
output_df$Alias<-rep(paste0("'",alias,"'"),nrow(output_df))

nc<-ncol(merged_df)
output_df<-cbind(output_df,merged_df[,3:nc])
names(output_df)<-c(label,"#","BgnDateTime","EndDateTime","Serial","MpCnt","Rg","PgCnt","LdCnt","Alias",metrics)
