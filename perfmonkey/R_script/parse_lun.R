...
output_df$DefMp<-rep(paste0("'",defmp,"'"),nrow(output_df))
output_df$DaCnt<-rep(paste0("'",dacnt,"'"),nrow(output_df))
output_df$Rg<-rep(paste0("'",rg,"'"),nrow(output_df))
...
names(output_df)<-c(label,"#","BgnDateTime","EndDateTime","Serial","DefMp","DaCnt","Rg","Ld","Alias",metrics)
