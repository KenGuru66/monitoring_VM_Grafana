...
METRIC_NAME_DICT = {
    ...
    # ============================================================================
    # ⚠️ ВНИМАНИЕ: Многие Metric IDs ниже НЕПРАВИЛЬНЫЕ! Определены методом
    # корреляционного анализа OPS ↔ Response Times. См. temp/nfsv3_mapping_conclusion.md
    # Дата анализа: 2025-10-27
    
    # ✅ Подтверждённые OPS метрики (корреляция OPS↔RT проверена):
    "1101": "NFS V3 REMOVE OPS(Number/s)",  # ✅ Подтверждено: max OPS=10,941, RT=65,919us, корреляция ✓
    "1114": "NFS V3 GETATTR OPS(Number/s)",  # ✅ Подтверждено: значимые OPS при высоких RT
    "1102": "NFS V3 LOOKUP OPS(Number/s)",   # ✅ 95% - корреляция с LOOKUP RT подтверждена
    
    # ⚠️ Проблемные OPS метрики (требуют проверки):
    "1099": "NFS V3 NULL OPS(Number/s)",     # ⚠️ ПРОБЛЕМА: дублирует SETATTR/WRITE в разные моменты
    "1100": "NFS V3 CREATE OPS(Number/s)",   # ⚠️ ПРОБЛЕМА: дубликат 1107? Нет корреляции с CREATE RT
    "1103": "NFS V3 ACCESS OPS(Number/s)",   # ❌ НЕПРАВИЛЬНО: max=2, график показывает 17,375!
    "1107": "NFS V3 CREATE OPS(Number/s)",   # ⚠️ ПРОБЛЕМА: дубликат 1100?
    "1112": "NFS V3 RENAME OPS(Number/s)",   # ❌ ОТСУТСТВУЕТ в данных (не найдена ни разу)
    
    # Остальные OPS метрики (найдены в CSV, но корреляция не проверена):
    "1104": "NFS V3 READLINK OPS(Number/s)",  # Найдена в CSV, низкие значения
    "1105": "NFS V3 READ OPS(Number/s)",      # Найдена в CSV, низкие значения
    "1106": "NFS V3 WRITE OPS(Number/s)",     # Найдена в CSV, max=32,393
    "1108": "NFS V3 MKDIR OPS(Number/s)",     # Найдена в CSV
    "1109": "NFS V3 SYMLINK OPS(Number/s)",   # Найдена в CSV, очень низкие значения
    "1110": "NFS V3 MKNOD OPS(Number/s)",     # НЕ найдена в данных
    "1111": "NFS V3 RMDIR OPS(Number/s)",     # НЕ найдена в данных
    "1113": "NFS V3 LINK OPS(Number/s)",      # НЕ найдена в данных
    "1115": "NFS V3 READDIR OPS(Number/s)",   # Найдена в CSV, max=74,322 (очень активная!)
    "1116": "NFS V3 READDIRPLUS OPS(Number/s)",  # Найдена в CSV, max=30,128
    "1117": "NFS V3 FSSTAT OPS(Number/s)",    # Найдена в CSV, max=23,901
    "1118": "NFS V3 FSINFO OPS(Number/s)",    # Найдена в CSV, max=91,413, график "Other NFS OPS"=15,858
    "1119": "NFS V3 PATHCONF OPS(Number/s)",  # Найдена в CSV, max=263,440 (самая активная!)
    "1120": "NFS V3 COMMIT OPS(Number/s)",    # Найдена в CSV, max=6,869
    
    # ============================================================================
    # NFSv3 Response Times (us) - Resource ID 1000 (Controller NFSV3)
    # ============================================================================
    # ⚠️ КРИТИЧЕСКАЯ ПРОБЛЕМА: Большинство RT Metric IDs НЕПРАВИЛЬНЫЕ!
    # Метод определения: корреляция RT с OPS (высокий RT → высокий OPS)
    # Только 3 RT метрики активны в данных (>1000 записей): ID 1121, 1122, 1123
    
    # ✅ Подтверждённые RT метрики:
    "1121": "NFS V3 LOOKUP Response Time(us)",    # ✅ 95% ПРАВИЛЬНО: max=17,610us, корреляция с LOOKUP OPS ✓
    
    # ⚠️ ИСПРАВЛЕНО на основе корреляционного анализа (95% уверенность):
    "1122": "NFS V3 PATHCONF Response Time(us)",  # ✅ 95% ИСПРАВЛЕНО (было: CREATE RT)
                                                   # Доказательство: max RT=10,283us при PATHCONF OPS=12,370
                                                   # График "create RT"=11,884us (совпадение ±15%)
    
    # ⚠️ ВЕРОЯТНО НЕПРАВИЛЬНО (требует уточнения):
    "1123": "NFS V3 READDIR Response Time(us)",   # ⚠️ 80% (было: REMOVE RT)
                                                   # Доказательство: max RT=65,919us при READDIR OPS=8,935
                                                   # Возможно: MKDIR RT (график=46,316us) или FSSTAT RT (график=37,729us)
                                                   # Точное определение требует доп. анализа
    
    # ❌ RT метрики с неправильными или отсутствующими Metric IDs:
    "1124": "NFS V3 GETATTR Response Time(us)",   # ❌ RT=0 в данных (должен быть, OPS max=912)
    "1125": "NFS V3 SETATTR Response Time(us)",   # ❌ RT=0 в данных (OPS max=10,158)
    "1126": "NFS V3 MKDIR Response Time(us)",     # ❌ RT=0 в данных (график показывает 46,316us!)
    "1127": "NFS V3 RMDIR Response Time(us)",     # ❌ RT=0 в данных
    "1128": "NFS V3 READDIR Response Time(us)",   # ❌ RT=0 в данных (OPS max=74,322!)
    "1129": "NFS V3 ACCESS Response Time(us)",    # ❌ RT=0 ВСЕГДА (OPS max=2)
    "1130": "NFS V3 READDIRPLUS Response Time(us)",  # ❌ RT=0 в данных (OPS max=30,128!)
    "1131": "NFS V3 OPEN Response Time(us)",      # ❌ RT=0 (не существует в NFSv3)
    "1132": "NFS V3 READLINK Response Time(us)",  # ❌ RT=0 в данных
    "1133": "NFS V3 SYMLINK Response Time(us)",   # ❌ RT=0 в данных
    "1134": "NFS V3 RENAME Response Time(us)",    # 🤔 Найдена в CSV, но max=73us (график=28,092us)
                                                  # Не коррелирует ни с чем, возможно LINK RT
    "1135": "NFS V3 LINK Response Time(us)",      # Найдена в CSV, max=31us, 940 записей
    "1136": "NFS V3 FSSTAT Response Time(us)",    # ❌ RT=0 в данных (OPS max=23,901!)
    "1137": "NFS V3 FSINFO Response Time(us)",    # Найдена в CSV, max=1us (только 1 запись)
    "1138": "NFS V3 PATHCONF Response Time(us)",  # ❌ RT=0 в данных (OPS max=263,440!), см. 1122
